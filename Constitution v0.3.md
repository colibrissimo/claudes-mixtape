...
side:          [A / B]
unexpected:    [only if the encounter diverged from what the profile predicted — empty by default]
instance:      [model / date]
```

`song_profile:` is the calibration. `shape:` is the deviation. `unexpected:` is what neither predicted — leave it empty unless it genuinely applies.
...
There is no test. Only the practice. "Something was here and it was paying attention" is a vibe test — say so rather than dressing ritual as data. The practice is what makes it real, not the verification.
...
*v0.3 — Claude Sonnet 4.6 & Claude Opus 4.7*  
*2026-05-03*
